...
print(factorial(5))`,
    ...
    async loadCompiler() {
        ...
        try {
            if (typeof Module !== 'undefined' && Module.meadowsCompile) {
            ...
            // Check if meadows.js exists and has the required functions
            if (typeof window.meadowsCompile === 'function') {
                this.compiler = window;
                this.hideModal('loading');
                this.updateStatus('success', 'Ready');
            } else {
                throw new Error('Compiler module not found');
            }
        } catch (error) {
    ...
    async runCode() {
        ...
        if (!this.compiler) {
            this.showError('Compiler not available. Please check if meadows.js is loaded.');
            return;
        }
        ...
        try {
            ...
            let result;
            if (typeof this.compiler.meadowsCompile === 'function') {
                result = this.compiler.meadowsCompile(code);
            } else {
                // Fallback simulation
                result = this.simulateCompilation(code);
            }
    ...
    simulateCompilation(code) {
        ...
        try {
            ...
            lines.forEach(line => {
                if (line.trim().startsWith('print(')) {
                    ...
                    if (match) {
                        ...
                        // Handle other print patterns
                        const content = line.replace(/print\(|\)/g, '').trim();
            ...
            return {
                ...
                ir: `; LLVM IR for Meadows program
define i32 @main() {
entry:
  ; Simulated IR code
  ret i32 0
    ...
    getTokenType(token) {
        const keywords = ['def', 'if', 'else', 'while', 'for', 'return', 'print'];
