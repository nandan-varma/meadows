...
void Lexer::handleIndentation() {
    ...
    if (currentIndent > previousIndent) {
        ...
        while (!indentStack.empty() && indentStack.top() > currentIndent) {
            indentStack.pop();
            pendingTokens.push_back(Token(TokenType::DEDENT, "", getCurrentLocation()));
        }
        
        if (indentStack.empty() || indentStack.top() != currentIndent) {
            // Indentation error
            pendingTokens.push_back(Token(TokenType::UNKNOWN, "Indentation error", getCurrentLocation()));
        }
    }
}
...
std::string Lexer::getErrorContext(const SourceLocation& location, int contextLines) const {
    ...
    int endLine = std::min(static_cast<int>(lines.size()), location.line + contextLines);
    
    for (int i = startLine; i <= endLine; i++) {
        result << i << ": " << lines[i - 1] << "\n";
        if (i == location.line) {
            result << std::string(std::to_string(i).length() + 2 + location.column - 1, ' ') << "^\n";
        }
    }
