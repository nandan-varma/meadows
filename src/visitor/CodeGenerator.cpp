...
CodeGenerator::CodeGenerator(ErrorReporter &errorReporter,
    ...
    : errorReporter(errorReporter), currentFunction(nullptr),
      lastValue(nullptr) {
...
llvm::Function *CodeGenerator::getOrCreatePrintfFunction() {
  ...
  if (!printfFunc) {
    llvm::FunctionType *printfType =
        llvm::FunctionType::get(getInt32Type(), {getInt8PtrType()},
                                true // varargs
        );
...
llvm::Value *
CodeGenerator::createPrintfCall(const std::string &format,
  ...
  llvm::Constant *formatStr =
      llvm::ConstantDataArray::getString(module->getContext(), format, true);
  ...
  llvm::Value *formatPtr =
      builder->CreateConstGEP2_32(formatStr->getType(), formatGlobal, 0, 0, "");
...
bool CodeGenerator::generateObjectFile(const std::string &filename) {
  try {
    ...
    std::vector<llvm::StringRef> args = {"clang", "-c", irFilename, "-o",
                                         filename};

    std::string errorMsg;
    int result =
        llvm::sys::ExecuteAndWait(llvm::sys::findProgramByName("clang").get(),
                                  args, std::nullopt, {}, 0, 0, &errorMsg);
    ...
    errorReporter.error("Exception in generateObjectFile: " +
                            std::string(e.what()),
                        SourceLocation(1, 1, ""));
    return false;
  } catch (...) {
...
bool CodeGenerator::generateExecutable(const std::string &objectFile,
                                       const std::string &executableFile) {
  // Use clang to link the object file
  std::vector<llvm::StringRef> args = {"clang", objectFile, "-o",
                                       executableFile};
...
void CodeGenerator::visit(StringLiteral &node) {
  ...
  // Get pointer to the string
  lastValue =
      builder->CreateConstGEP2_32(strConstant->getType(), strGlobal, 0, 0, "");
}
...
void CodeGenerator::visit(ListLiteral &node) {
  ...
  if (node.elements.empty()) {
    ...
    lastValue = llvm::ConstantPointerNull::get(
        llvm::PointerType::get(getInt32Type(), 0));
  } else {
    ...
    for (auto &element : node.elements) {
      ...
      if (lastValue && llvm::isa<llvm::Constant>(lastValue)) {
        ...
        errorReporter.error(
            "Non-constant elements in list literals not yet supported",
            node.location);
        lastValue = nullptr;
        return;
      }
    ...
    llvm::ArrayType *arrayType =
        llvm::ArrayType::get(getInt32Type(), elements.size());
...
void CodeGenerator::visit(FunctionCall &node) {
  ...
  if (auto identifierFunc = dynamic_cast<Identifier *>(node.function.get())) {
    ...
    if (!function) {
      errorReporter.error("Unknown function: " + identifierFunc->name,
                          node.location);
      lastValue = nullptr;
      return;
    }
    ...
    errorReporter.error("Function calls only supported for identifiers",
                        node.location);
    lastValue = nullptr;
  }
}
...
void CodeGenerator::visit(AttributeAccess &node) {
  ...
  errorReporter.error("Attribute access not yet implemented in code generation",
                      node.location);
  lastValue = nullptr;
}

void CodeGenerator::visit(IndexAccess &node) {
  errorReporter.error("Index access not yet implemented in code generation",
                      node.location);
  lastValue = nullptr;
}
...
void CodeGenerator::visit(Assignment &node) {
  if (auto identifier = dynamic_cast<Identifier *>(node.target.get())) {
    ...
    if (!variable) {
      ...
      variable =
          tmpBuilder.CreateAlloca(value->getType(), nullptr, identifier->name);
    ...
    errorReporter.error("Assignment only supported for identifiers",
                        node.location);
    lastValue = nullptr;
  }
}
...
void CodeGenerator::visit(IfStatement &node) {
  ...
  if (!condition->getType()->isIntegerTy(1)) {
    ...
    condition = builder->CreateICmpNE(
        condition, llvm::ConstantInt::get(getInt32Type(), 0), "ifcond");
  }
...
void CodeGenerator::visit(WhileStatement &node) {
  ...
  if (!condition->getType()->isIntegerTy(1)) {
    condition = convertToInt(condition);
    condition = builder->CreateICmpNE(
        condition, llvm::ConstantInt::get(getInt32Type(), 0), "whilecond");
  }
...
void CodeGenerator::visit(ForStatement &node) {
  errorReporter.error("For loops not yet implemented in code generation",
                      node.location);
}
...
void CodeGenerator::visit(BreakStatement &node) {
  errorReporter.error("Break statements not yet implemented in code generation",
                      node.location);
}

void CodeGenerator::visit(ContinueStatement &node) {
  errorReporter.error(
      "Continue statements not yet implemented in code generation",
      node.location);
}
...
void CodeGenerator::visit(FunctionDefinition &node) {
  ...
  for (const auto &param : node.parameters) {
    paramTypes.push_back(
        getInt32Type()); // Simplified: all parameters are int32
  }

  llvm::FunctionType *funcType =
      llvm::FunctionType::get(getInt32Type(), paramTypes, false);
  ...
  for (const auto &param : node.parameters) {
    llvm::AllocaInst *alloca =
        builder->CreateAlloca(getInt32Type(), nullptr, param.name);
...
void CodeGenerator::visit(ClassDefinition &node) {
  errorReporter.error(
      "Class definitions not yet implemented in code generation",
      node.location);
}
...
void CodeGenerator::visit(Program &node) {
  ...
  llvm::FunctionType *mainType =
      llvm::FunctionType::get(getInt32Type(), {}, false);
